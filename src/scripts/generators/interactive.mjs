...
import { loadScaffolderAdapter } from '../scaffolder/adapters/detect.mjs'
...
let fieldsDir = path.join(rootDir, 'src/cms/fields')
let blocksDir = path.join(rootDir, 'src/cms/blocks')
let blockComponentsDir = path.join(rootDir, 'src/components/blocks')

const resolvePath = (relativePath) => path.join(rootDir, relativePath)

const initScaffolderContext = async ({ adapter: providedAdapter, dryRun = false, cwd = process.cwd() } = {}) => {
  rootDir = cwd
  adapter = providedAdapter ?? (await loadScaffolderAdapter(rootDir))
  runtimeOptions = { dryRun }
  fieldsDir = resolvePath(adapter.paths.fieldsDir)
  blocksDir = resolvePath(adapter.paths.blocksDir)
  blockComponentsDir = resolvePath(adapter.paths.blockComponentsDir)
}
...
const withGeneratedHeader = (content) => {
  const header = adapter?.generators?.defaultHeaderComment
  if (!header) return content
  if (content.startsWith('/**\n * Generated by local scaffolder.')) return content
  return `/**\n * ${header}\n */\n\n${content}`
}

const writeFile = (filePath, content, options = {}) => {
  const nextContent = options.generated ? withGeneratedHeader(content) : content
  if (runtimeOptions.dryRun) {
    process.stdout.write(`[dry-run] write ${path.relative(rootDir, filePath)}\n`)
    return
  }
...
const upsertLine = (filePath, line, options = {}) => {
  ...
  if (runtimeOptions.dryRun) {
    process.stdout.write(`[dry-run] update ${path.relative(rootDir, filePath)} -> ${normalizedLine}\n`)
    return
  }
  ...
  return fs
    ...
    .map((entry) => {
      ...
      const configPath = entry.isDirectory()
        ? fs.existsSync(path.join(blocksDir, entry.name, 'config.ts'))
          ? path.join(blocksDir, entry.name, 'config.ts')
          : fs.existsSync(path.join(blocksDir, entry.name, 'index.ts'))
            ? path.join(blocksDir, entry.name, 'index.ts')
            : null
...
const getExistingRegistryEntries = () => {
  const registryPath = resolvePath(adapter.paths.blockRegistry)
...
const assertBlockDoesNotExist = ({ exportName, componentName, pascalName, slug }) => {
  ...
  if (registryCollision) {
    throw new Error(`Block renderer already exists for key "${rendererKey}" in ${adapter.paths.blockRegistry}.`)
  }
}
...
const normalizeBlocksIndex = (filePath) => {
  ...
  if (runtimeOptions.dryRun) {
    process.stdout.write(`[dry-run] normalize ${path.relative(rootDir, filePath)}\n`)
    return
  }
...
const updateCmsBlocksIndex = (config) => {
  const filePath = resolvePath(adapter.paths.blocksIndex)
  ...
  const source = readFile(filePath)
  const pattern = new RegExp(`export const ${adapter.registration.blockArrayName} = \\[([\\s\\S]*?)\\] as const`)
  ...
  writeFile(
    ...
    source.replace(pattern, (_match, inner) => {
      ...
      return `export const ${adapter.registration.blockArrayName} = [${entries.join(', ')}] as const`
    }),
  )
...
const updateBlockComponentIndex = (config) => {
  upsertLine(resolvePath(adapter.paths.blockComponentIndex), `export * from './${config.componentName}'`)
}

const updateBlockRegistry = (config) => {
  const filePath = resolvePath(adapter.paths.blockRegistry)
  ...
  writeFile(
    filePath,
    source.replace(
      new RegExp(`export const ${adapter.registration.blockRegistryObjectName}: Record<string, BlockComponent> = \\{\\n`),
      (match) => `${match}${registryEntry}\n`,
    ),
  )
}

export const generateField = async (options = {}) => {
  await initScaffolderContext(options)
  ...
  try {
    ...
    const shouldExport = await prompt.confirm(`Export from ${adapter.paths.fieldsIndex}?`, true)
    ...
    writeFile(outputPath, buildFieldHelperFile(config), { generated: true })

    if (shouldExport) {
      upsertLine(resolvePath(adapter.paths.fieldsIndex), `export * from './${camelName}'`)
    }
...
export const generateBlock = async (options = {}) => {
  await initScaffolderContext(options)
  const prompt = createPrompt()
  ...
  try {
    ...
    writeFile(path.join(cmsBlockDir, 'config.ts'), buildBlockConfigFile({ exportName, fields, label, pascalName, slug }), { generated: true })
    writeFile(path.join(cmsBlockDir, 'index.ts'), buildBlockEntryFile({ exportName }), { generated: true })
...
export const runInteractiveCommand = async (command, options = {}) => {
  if (command === 'field') return generateField(options)
  if (command === 'block') return generateBlock(options)
  throw new Error('Usage: node src/scripts/generators/interactive.mjs <field|block> [--dry-run]')
}
...
if (isDirectRun && command) {
  runInteractiveCommand(command, { dryRun }).catch((error) => {
    const label = command === 'field' ? 'gen:field' : command === 'block' ? 'gen:block' : 'scaffolder'
